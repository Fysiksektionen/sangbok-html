...
function draw_staff(st,x1,x2){var w,i,dy,ty,y=0,ln="",stafflines=staff_tb[st].stafflines,l=stafflines.length,il=6*staff_tb[st].staffscale
if(!/[\[|]/.test(stafflines))
return
...
switch(cmd){case"aligncomposer":case"barsperstaff":case"infoline":case"measurenb":case"rbmax":case"rbmin":case"measrepnb":case"shiftunison":case"systnames":v=parseInt(param)
...
for(s2=s;s2&&s2.time==s.time;s2=s2.ts_next){switch(s2.type){case C.KEY:if(!s.fmt.keywarn||s2.invis)
continue
...
return s}
function cut_tune(lwidth,lsh){var s2,i,mc,pg_sav={leftmargin:cfmt.leftmargin,rightmargin:cfmt.rightmargin,pagewidth:cfmt.pagewidth,scale:cfmt.scale},indent=lsh[0]-lsh[1],ckw=get_ck_width(),s=tsfirst
lwidth-=lsh[1]
...
function get_lshift(){var st,v,p_v,p1,po,fnt,w,sy=cur_sy,lsh1=0,lsho=0,nv=voice_tb.length
...
if(fmt.systnames){for(v=voice_tb.length;--v>=0;){p_voice=voice_tb[v]
if(!cur_sy.voices[v]||!gene.st_print[p_voice.st])
continue
if(p_voice.nm&&(p_voice.new_name||fmt.systnames==2)){vnt=2
break}
if(p_voice.snm)
vnt=1}}
gene.vnt=vnt
...
if(s.k_a_acc){s.k_map=new Int8Array(s.k_map)
i=s.k_a_acc.length
...
if(!map[nn]){nn='k'+ntb[(note.pit+75-
p_v.ckey.k_sf*11)%7]
if(!map[nn]){nn='all'
if(!map[nn])
return}}}
...
s.ti2=true}
Abc.prototype.new_note=function(grace,sls){var note,s,in_chord,c,dcn,type,tie_s,acc_tie,i,n,s2,nd,res,num,dur,apit,div,ty,dpit=0,sl1=[],line=parse.line,a_dcn_sav=a_dcn
a_dcn=[]
...
var strwh
(function(){if(typeof document!="undefined"){var el
strwh=function(str){if(str.wh)
return str.wh
if(!el){el=document.createElement('text')
...
abc2svg.font_tb[i].used=false}else{style='';fulldefs=''}
defs='';posy=0
img.wx=0}
...
Abc.prototype.cfmt=function(){return cfmt};Abc.prototype.clone=clone;Abc.prototype.deco_put=function(nm,s){a_dcn.push(nm)
deco_cnv(s)}
