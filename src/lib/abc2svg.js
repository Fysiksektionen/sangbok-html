...
abc2svg.Abc = function (user) {
    "use strict"; var C = abc2svg.C; var require = empty_function, system = empty_function, write = empty_function, XMLHttpRequest = empty_function, std = null, os = null
    ...
    function scanBuf() {
        this.index = 0; scanBuf.prototype.char = function () { return this.buffer[this.index] }
    ...
    function syntax(sev, msg, a1, a2, a3, a4) {
        var s = { fname: parse.fname, istart: parse.istart + parse.line.index }
    ...
    function y_get(st, up, x, w) {
        var y, p_staff = staff_tb[st], i = (x / 2) | 0, j = ((x + w) / 2) | 0
        if (i < 0)
            i = 0
        if (j >= YSTEP) {
            j = YSTEP - 1
    ...
    function y_set(st, up, x, w, y) {
        var p_staff = staff_tb[st], i = (x / 2) | 0, j = ((x + w) / 2) | 0
        if (i < 0)
            i = 0
        if (j >= YSTEP) {
            j = YSTEP - 1
    ...
    function d_arp(de) {
        var m, h, dx, s = de.s, dd = de.dd, xc = dd.wr
        if (s.type == C.NOTE) {
            for (m = 0; m <= s.nhd; m++) {
                if (s.notes[m].acc) { dx = s.notes[m].shac } else {
                    dx = 1 - s.notes[m].shhd
                    switch (s.head) {
                        case C.SQUARE: dx += 3.5
                            break
    ...
    function d_near(de) {
        var y, up = de.up, s = de.s, dd = de.dd
        y = up ? s.ymx : s.ymn
    ...
    function d_slide(de) {
        ...
        for (m = 0; m <= s.nhd; m++) {
            if (s.notes[m].acc) { dx = 4 + s.notes[m].shac } else {
                dx = 5 - s.notes[m].shhd
                switch (s.head) {
                    case C.SQUARE: dx += 3.5
                        break
    ...
    function d_upstaff(de) {
        ...
        switch (dd.glyph) {
            case "lphr": case "mphr": case "sphr": case "short": case "tick": if (s.type == C.BAR)
                s.invis = 1
        ...
        if (s.nhd)
            x += s.notes[s.stem >= 0 ? 0 : s.nhd].shhd; switch (dd.ty) {
                case '@': case '<': case '>': y = de.y
                    break
            }
        if (y == undefined) {
            if (up) {
    ...
    function do_ctie(nm, s, nt1) {
        ...
        cross[nm2] = null
        if (nt1.midi != nt2.midi || nt1.s.time + nt1.s.dur != nt2.s.time) { error(1, s, "Bad tie") } else {
    ...
    function deco_cnv(s, prev) {
        ...
        while (1) {
            ...
            if (!dd)
                continue
            switch (dd.func) {
                case 0: if (s.type == C.BAR && nm == "dot") {
                    s.bar_dotted = true
                    continue
                }
                case 1: case 2: if (!s.notes) {
                    error(1, s, errs.must_note_rest, nm)
                    continue
                }
                    break
                case 3: if (fg && dd.glyph == "fng") {
                    for (i = 0; i <= 5; i++) {
                ...
                case 32: s.invis = true
                    break
                case 33: if (s.type != C.BAR) {
                    error(1, s, "!beamon! must be on a bar")
                    continue
                }
                    s.beam_on = true
                    break
                case 34: if (s.type != C.NOTE || !prev || prev.type != C.NOTE || s.dur != prev.dur) {
                    ...
                    s.trem2 = true; s.beam_end = true; s.beam_st = false; prev.beam_st = true; prev.beam_end = false; s.ntrem = prev.ntrem = Number(nm[4]); for (j = 0; j <= s.nhd; j++)
                        ...
                        prev.notes[j].dur *= 2
                    break
                case 35: if (s.type != C.NOTE) {
                    error(1, s, errs.must_note, nm)
                    continue
                }
                    s.xstem = true; break
                case 36: if (s.type != C.NOTE) {
                    error(1, s, errs.must_note, nm)
                    continue
                }
                    if (nm[6] == '1')
                ...
                case 37: s.rbstop = 1
                    break
                case 38: if (s.type != C.NOTE) {
                    error(1, s, errs.must_note, nm)
                    continue
                }
                    s.trem1 = true; s.ntrem = nm.length
                    break
                case 39: if (s.type != C.NOTE) {
                    error(1, s, errs.must_note, nm)
                    continue
                }
    ...
    function dh_cnv(s, nt) {
        ...
        while (1) {
            ...
            if (!dd)
                continue
            switch (dd.func) {
                case 0: case 1: case 3: case 4: case 8: break
    ...
    function deco_width(s, wlnt) {
        var dd, i, w, wl = wlnt, wr = s.wr, a_dd = s.a_dd, nd = a_dd.length
        for (i = 0; i < nd; i++) {
            dd = a_dd[i]
            switch (dd.func) {
                case 1: case 2: if (wl < 12)
                    wl = 12
                    break
                case 3: switch (dd.glyph) {
                    case "brth": case "lphr": case "mphr": case "sphr": if (s.wr < 20)
                        s.wr = 20
                        break
                    default: w = dd.wl + 2
                        if (wl < w)
                ...
                default: switch (dd.ty) {
                    case '<': w = wlnt + dd.wl + dd.wr + 6
                        if (wl < w)
                            wl = w
                        break
    ...
    function deco_wch(nt) {
        var i, w, dd, wl = 0, n = nt.a_dd.length
        for (i = 0; i < n; i++) {
            dd = nt.a_dd[i]
            if (dd.ty == '<') {
                w = dd.wl + dd.wr + 4
    ...
    Abc.prototype.draw_all_deco = function () {
        ...
        function out_fg() {
            var k, l, de2, fg, fg2, x2, j = s.fg.length
            while (--j >= 0) {
                fg = s.fg[j]
                if (fg.nm == dd.name)
                    break
        ...
        while (1) {
            ...
            if (de.m != undefined) {
                note = s.notes[de.m]; if (note.shhd)
            ...
            } else if (dd.str != undefined && !tgls[dd.glyph] && !glyphs[dd.glyph]) {
                if (s.fg)
                    out_fg()
                out_deco_str(x, y, de)
            } else if (de.lden) { out_deco_long(x, y, de) } else { xygl(x, y, f) }
            if (stv_g.g)
    ...
    function draw_deco_near() {
        ...
        function create_deco(s) {
            ...
            for (k = 0; k < nd; k++) {
                ...
                if (pos)
                    de.pos = pos
                up = 0
                if (dd.ty == '^') { up = 1 } else if (dd.ty == '_') { } else {
                    switch (dd.func) {
                        case 0: if (s.multi)
                            up = s.multi > 0
                        else
        ...
        function create_dh(s, m) {
            var de, k, dd, note = s.notes[m], nd = note.a_dd.length, x = s.x
            for (k = 0; k < nd; k++) {
                dd = note.a_dd[k]
        ...
        function ll_deco() {
            ...
            for (i = 0; i < n_de; i++) {
                ...
                if (j == n_de) {
                    de2 = { s: s, st: de.st, dd: dd2, ix: a_de.length - 1, x: realwidth - 6, y: s.y, cont: true, lden: true, defl: { noen: true } }
                    if (de2.x < s.x + 10)
                        de2.x = s.x + 10
                    if (de.m != undefined)
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            switch (s.type) { case C.CLEF: case C.KEY: case C.METER: continue }
            break
        }
        if (a_de.length)
            ldeco_update(s)
        for (; s; s = s.ts_next) {
            switch (s.type) {
                case C.BAR: case C.MREST: case C.NOTE: case C.REST: case C.SPACE: break
    ...
    function draw_deco_staff() {
        var s, p_voice, y, i, v, de, dd, w, minmax = new Array(nstaff + 1), nd = a_de.length
        function draw_repbra(p_voice) {
            var s, s1, x, y, y2, i, p, w, wh, first_repeat; y = staff_tb[p_voice.st].topbar + 15
            for (s = p_voice.sym; s; s = s.next) {
                ...
                if (s1.text) {
                    wh = strwh(s1.text); y2 = y_get(p_voice.st, true, x + 4, wh[0]) +
                        wh[1]
                    if (y < y2)
                        y = y2
                }
            ...
            for (; s; s = s.next) {
                ...
                if (cfmt.measurenb > 0 & s.bar_num && s.bar_num % cfmt.measurenb)
                    x += 6
                if (s.type != C.BAR) { w = s.rbstop ? 0 : s.x - realwidth + 4 } else if ((s.bar_type.length > 1 && s.bar_type != "[]") || s.bar_type == "]") {
    ...
    function draw_measnb() {
        ...
        if (staff_tb[st].staffscale != 1) {
            font_size = get_font("measure").size; param_set_font("measurefont", "* " +
        ...
        if (bar_num > 1) {
            if (cfmt.measurenb == 0) {
                ...
                for (; ; s = s.ts_next) {
                    switch (s.type) { case C.CLEF: case C.KEY: case C.METER: case C.STBRK: continue }
                    break
                }
    ...
    function draw_partempo() {
        var s, s2, some_part, some_tempo, h, w, y, st, sy = cur_sy
        for (st = 0; st <= nstaff; st++) {
            if (sy.st_print[st])
                break
        }
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if (!s2 || s2.invis)
                continue
            if (!some_part) {
                some_part = s; set_font("parts"); h = gene.curfont.size + 2 +
                    gene.curfont.pad * 2
            }
            if (s2.x == undefined)
                s2.x = s.x - 10
    ...
    function b_pos(grace, stem, nflags, b) {
        var top, bot, d1, d2, shift = !grace ? BEAM_SHIFT : 3.5, depth = !grace ? BEAM_DEPTH : 1.8
        function rnd6(y) {
    ...
    Abc.prototype.calculate_beam = function (bm, s1) {
        var s, s2, g, notes, nflags, st, v, two_staves, two_dir, n, x, y, ys, a, b, stem_err, max_stem_err, p_min, p_max, s_closest, stem_xoff, scale, visible, dy
        if (!s1.beam_st) {
            s = sym_dup(s1); lkvsym(s, s1); lktsym(s, s1); s.x -= 12
            if (s.x > s1.prev.x + 12)
        ...
        if (two_dir) {
            ...
            for (; s && s.time <= s2.time; s = s.ts_next) {
                if (s.type != C.NOTE || s.invis || (s.st != st && s.v != v)) { continue }
            ...
            for (; ; s = s.next) {
                ys = a * s.xs + b - staff_tb[s.st].y; stem_err = GSTEM - 2
                if (s.stem > 0)
                    stem_err -= ys - (3 * (s.notes[s.nhd].pit - 18))
                else
        ...
        for (s = s1; ; s = s.next) {
            switch (s.type) {
                case C.NOTE: s.ys = a * s.xs + b - staff_tb[s.st].y
    ...
    function draw_beams(bm) {
        var s, i, beam_dir, shift, bshift, bstub, bh, da, bsh, k, k1, k2, x1, osh = 0, s1 = bm.s1, s2 = bm.s2
        function draw_beam(x1, x2, dy, h, bm, n) {
            var y1, dy2, s = bm.s1, nflags = s.nflags
            if (s.ntrem)
                nflags -= s.ntrem
            if (s.trem2 && n > nflags) {
                if (s.dur >= C.BLEN / 2) { x1 = s.x + 6; x2 = bm.s2.x - 6 } else if (s.dur < C.BLEN / 4) {
                    var dx = x2 - x1
                    if (dx < 16) {
                        x1 += dx / 4
                        x2 -= dx / 4
        ...
        for (i = 2; i <= bm.nflags; i++) {
            ...
            for (s = s1; ; s = s.next) {
                ...
                if (s.trem1 && i > s.nflags - s.ntrem) {
                    x1 = (s.dur >= C.BLEN / 2) ? s.x : s.xs; draw_beam(x1 - 5, x1 + 5, (shift + 2.5) * beam_dir, bh, bm, i)
                    if (s == s2)
                        break
                    continue
                }
                ...
                bsh = shift * beam_dir
                if (k1 == k2) {
                    if (k1 == s1) { x1 += bstub } else if (k1 == s2) { x1 -= bstub } else if (k1.beam_br1 || (k1.beam_br2 && i > 2)) { x1 += bstub } else {
                        k = k1.next
                        while (k.type != C.NOTE)
                            k = k.next
                        if (k.beam_br1 || (k.beam_br2 && i > 2)) { x1 -= bstub } else {
                            k1 = k1.prev
    ...
    function draw_lstaff(x) {
        var i, j, yb, h, fl, nst = cur_sy.nstaff, l = 0
    ...
    function draw_meter(s) {
        ...
        for (i = 0; i < s.a_meter.length; i++) {
            meter = s.a_meter[i]; x = s.x + s.x_meter[i]
            if (meter.bot) {
                out_XYAB('\
...
<text x="X" y="Y" text-anchor="middle">A</text>\n', x, y + 12, m_gl(meter.top))
    ...
    function set_hl(p_st, n, x, dx1, dx2) {
        ...
        if (i == hl.length) { hl.push([x, dx1, dx2]) } else if (x > hl[i][0]) { hl.splice(++i, 0, [x, dx1, dx2]) } else {
            if (dx1 < hl[i][1])
                hl[i][1] = dx1
            if (dx2 > hl[i][2])
                hl[i][2] = dx2
    ...
    Abc.prototype.draw_hl = function (s) {
        var i, j, n, note, hla = [], st = s.st, p_staff = staff_tb[st]
    ...
    Abc.prototype.draw_keysig = function (x, s) {
        var old_sf = s.k_old_sf, st = s.st, staffb = staff_tb[st].y, i, shift, p_seq, clef_ix = s.k_y_clef, a_acc = s.k_a_acc
    ...
    function draw_rest(s) {
        var s2, i, j, y, bx, p_staff = staff_tb[s.st], yb = p_staff.y, x = s.x
        if (s.notes[0].shhd)
            x += s.notes[0].shhd * stv_g.scale
        if (s.rep_nb) {
            set_sscale(s.st); anno_start(s); if (p_staff.stafflines == '|||||')
                yb += 12
            else
                yb += (p_staff.topbar + p_staff.botbar) / 2
            if (s.rep_nb < 0) { xygl(x, yb, "srep") } else {
                xygl(x, yb, "mrep")
    ...
    function draw_mrest(s) {
        var x1, x2, prev, p_st = staff_tb[s.st], y = p_st.y + (p_st.topbar + p_st.botbar) / 2, p = s.nmes.toString()
        function omrest() {
            var x = s.x, y = p_st.y + 12, n = s.nmes, k = n >> 2
            if (n & 3) {
                k++
                if (n & 3 == 3)
                    k++
 ...
 text-anchor="middle">A</text>\n', s.x, y + 18, s.tacet)
    ...
    function grace_slur(s) {
        var yy, x0, y0, x3, y3, bet1, bet2, dy1, dy2, last, below, so = s, g = s.extra
        while (1) {
            if (!g.next)
                break
            g = g.next
    ...
    function draw_gracenotes(s) {
        var x1, y1, last, note, bm = {}, g = s.extra
        while (1) {
    ...
    function draw_basic_note(s, m, y_tb) {
        var i, p, yy, dotx, doty, inv, old_color = false, note = s.notes[m], staffb = staff_tb[s.st].y, x = s.x, y = 3 * (note.pit - 18), shhd = note.shhd * stv_g.scale, x_note = x + shhd, y_note = y + staffb
        var elts = identify_note(s, note.dur), head = elts[0], dots = elts[1], nflags = elts[2]
        if (note.invis) { } else if (s.grace) { p = "ghd"; x_note -= 4.5 * stv_g.scale } else if (note.map && note.map[0]) {
            i = head; p = note.map[0][i]
            if (!p)
        ...
        } else if (s.type == C.CUSTOS) { p = "custos" } else {
            switch (head) {
                case C.OVAL: p = "HD"
                    break
                case C.OVALBARS: if (s.head != C.SQUARE) {
                    p = "HDD"
                    break
                }
                case C.SQUARE: if (nflags > -4) { p = "breve" } else {
                    p = "longa"
                    inv = s.stem > 0
                }
        ...
        if (p) {
            if (inv) { g_open(x_note, y_note, 0, 1, -1); x_note = y_note = 0 }
        ...
        if (note.acc) {
            ...
            if (!s.grace) { draw_acc(x, y + staffb, note.acc) } else {
                g_open(x, y + staffb, 0, .75); draw_acc(0, 0, note.acc)
                g_close()
            }
        }
        if (old_color != false)
    ...
    function draw_note(s, fl) {
        var s2, i, m, y, slen, c, nflags, y_tb = new Array(s.nhd + 1), note = s.notes[s.stem < 0 ? s.nhd : 0], x = x_head(s, note), y = y_head(s, note), staffb = staff_tb[s.st].y
        if (s.dots)
            setdoty(s, y_tb)
        if (!s.stemless) {
            slen = s.ys - s.y; nflags = s.nflags
            if (s.ntrem)
                nflags -= s.ntrem
            if (!fl || nflags <= 0) {
                if (s.nflags > 0) {
            ...
            } else { out_stem(x, y, slen, s.grace, nflags, s.fmt.straightflags) }
        } else if (s.xstem) { s2 = s.ts_prev; slen = (s2.stem > 0 ? s2.y : s2.ys) - s.y; slen += staff_tb[s2.st].y - staffb; out_stem(x, y, slen) }
        if (fl && s.trem1) {
            var ntrem = s.ntrem || 0, x1 = x; slen = 3 * (s.notes[s.stem > 0 ? s.nhd : 0].pit - 18)
    ...
    function slur_out(x1, y1, x2, y2, dir, height, dotted) {
        var dx, dy, dz, alfa = .3, beta = .45; dy = y2 - y1
        if (dy < 0)
            dy = -dy; dx = x2 - x1
        if (dx > 40. && dy / dx < .7) {
            alfa = .3 + .002 * (dx - 40.)
    ...
    function draw_slur(path, sl, recurr) {
        var i, k, g, x1, y1, x2, y2, height, addy, s_st2, a, y, z, h, dx, dy, ty = sl.ty, dir = (ty & 0x07) == C.SL_ABOVE ? 1 : -1, n = path.length, i1 = 0, i2 = n - 1, not1 = sl.nts, k1 = path[0], k2 = path[i2], nn = 1
        set_dscale(k1.st)
        for (i = 1; i < n; i++) {
            k = path[i]
            if (k.type == C.NOTE || k.type == C.REST) {
        ...
        if (not1) {
            ...
            if (k1.type == C.NOTE) {
                if (dir > 0) {
                    if (k1.stem > 0) {
                        x1 += 5
                        if (k1.beam_end && k1.nflags >= -1 && !k1.in_tuplet) { if (k1.nflags > 0) { x1 += 2; y1 = k1.ys - 3 } else { y1 = k1.ys - 6 } } else { y1 = k1.ys + 3 }
                    } else { y1 = k1.y + 8 }
                } else {
                    if (k1.stem < 0) {
                        x1 -= 1
                        if (k2.grace) { y1 = k1.y - 8 } else if (k1.beam_end && k1.nflags >= -1 && (!k1.in_tuplet || k1.ys < y1 + 3)) { if (k1.nflags > 0) { x1 += 2; y1 = k1.ys + 3 } else { y1 = k1.ys + 6 } } else { y1 = k1.ys - 3 }
                    } else { y1 = k1.y - 8 }
                }
            }
        }
        ...
        if (k1 != k2 && k1.v == k2.v) {
            ...
            for (i = 1; i < i2; i++) {
                k = path[i]
                switch (k.type) {
                    case C.NOTE: case C.REST: if (dir > 0) {
                        ...
                        break
                    case C.GRACE: for (g = k.extra; g; g = g.next) {
                        if (dir > 0) {
    ...
    function draw_slurs(s, last) {
        ...
        function draw_sls(s, sl) {
            var k, v, i, dir, s3, path = [], s2 = sl.se
            if (last && s2.time > last.time)
                return
            switch (sl.loc) {
                case 'i': s = prev_scut(s)
                    break
            ...
            if (s.p_v.s_next && s2.time >= tsnext.time) {
                ...
                if (!s3) {
                    s.p_v.sls.push(sl); s2 = s.p_v.s_next.prev
                    while (s2.next)
            ...
            if (s.v == s2.v) { v = s.v } if (!cur_sy.voices[s.v] || !cur_sy.voices[s2.v]) { v = s.v > s2.v ? s.v : s2.v } else if (dir * (cur_sy.voices[s.v].range <= cur_sy.voices[s2.v].range ? 1 : -1) > 0)
                ...
                v = s2.v
            if (gr1 && !(s2.grace && s.v == s2.v && s.time == s2.time)) { do { path.push(s); s = s.next } while (s); s = gr1.next } else {
                path.push(s); if (s.grace)
                    s = s.next
                else
                    s = s.ts_next
            }
    ...
    function draw_tuplet(s1) {
        var s2, s3, g, stu, std, nb_only, x1, x2, y1, y2, xm, ym, a, s0, yy, yx, dy, a, dir, r, tp = s1.tp.shift()
        if (!s1.tp.length)
            delete s1.tp
        stu = std = s1.st
        for (s2 = s1; s2; s2 = s2.next) {
            switch (s2.type) {
                case C.GRACE: if (!s2.sl1)
                    continue
        ...
        if (s1 == s2 || tp.f[1] == 2) { nb_only = true } else if (tp.f[1] == 1) { nb_only = true; draw_slur([s1, s2], { ty: dir }) } else {
        ...
        if (nb_only) {
            ...
            if (dir == C.SL_ABOVE) {
                ...
                if (s3.ymx < ym)
                    s3.ymx = ym; y_set(stu, 1, xm - 3, 6, ym)
            } else {
                if (s3.ymn > ym)
    ...
    function draw_tie(not1, not2, job) {
        var m, x1, s, y, h, time, p = job == 2 ? not1.pit : not2.pit, dir = (not1.tie_ty & 0x07) == C.SL_ABOVE ? 1 : -1, s1 = not1.s, st = s1.st, s2 = not2.s, x2 = s2.x, sh = not1.shhd
        for (m = 0; m < s1.nhd; m++)
            if (s1.notes[m] == not1)
                break
        if (dir > 0) {
        ...
        for (; s1; s1 = s1.next) {
            ...
            for (m = 0; m <= s1.nhd; m++) {
                ...
                if (clef_chg || s1.st != s2.st) {
                    ...
                    clef_chg = false
                } else { draw_tie(not1, not2) }
            }
        }
    }
    function draw_sym_near() {
        var p_voice, p_st, s, v, st, y, g, w, i, st, dx, top, bot, ymn, output_sav = output; function set_yab(s1, s2) {
            var y, k = realwidth / YSTEP, i = (s1.x / k) | 0, j = (s2.x / k) | 0, a = (s1.ys - s2.ys) / (s1.xs - s2.xs), b = s1.ys - s1.xs * a, p_st = staff_tb[s1.st]
            k *= a
            if (s1.stem > 0) {
                while (i <= j) {
                    y = k * i + b
        ...
        for (v = 0; v < voice_tb.length; v++) {
            var bm = {}, first_note = true; p_voice = voice_tb[v]
            for (s = p_voice.sym; s; s = s.next) {
                switch (s.type) {
                    case C.GRACE: for (g = s.extra; g; g = g.next) {
                        if (g.beam_st && !g.beam_end) {
            ...
            for (s = p_voice.sym; s; s = s.next) {
                switch (s.type) {
                    case C.NOTE: if ((s.beam_st && !s.beam_end) || (first_note && !s.beam_st)) {
                        first_note = false; self.calculate_beam(bm, s)
                        if (bm.s2)
                            set_yab(s, bm.s2)
                    }
                        break
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.invis)
                continue
            switch (s.type) {
                case C.GRACE: for (g = s.extra; g; g = g.next) { y_set(s.st, true, g.x - 2, 4, g.ymx + 1); y_set(s.st, false, g.x - 2, 4, g.ymn - 5) }
                    continue
            ...
            if (s.stem > 0) {
                if (s.stemless) { dx = -5; w = 10 } else if (s.beam_st) { dx = 3; w = s.beam_end ? 4 : 10 } else { dx = -8; w = s.beam_end ? 11 : 16 }
                ...
                y_set(s.st, true, s.x - s.wl, s.wl + s.wr, s.ymx); if (s.stemless) { dx = -5; w = 10 } else if (s.beam_st) { dx = -6; w = s.beam_end ? 4 : 10 } else { dx = -8; w = s.beam_end ? 5 : 16 }
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (!s)
                continue
            set_color(s.color); st = p_voice.st; for (; s; s = s.next) {
                if (s.play)
                    continue
                if (s.tp)
                    draw_tuplet(s)
    ...
    function set_staff() {
        var i, st, prev_staff, v, fmt, s, y, staffsep, dy, maxsep, mbot, val, p_voice, p_staff, sy = cur_sy
    ...
    function draw_systems(indent) {
        var s, s2, st, x, x2, res, sy, xstaff = [], stl = [], bar_bot = [], bar_height = [], ba = [], sb = "", thb = ""
        function bar_set() {
        ...
        function draw_staff(st, x1, x2) {
            var w, i, dy, ty, y = 0, ln = "", stafflines = staff_tb[st].stafflines, l = stafflines.length, il = 6 * staff_tb[st].staffscale
            if (!/[\[|]/.test(stafflines))
                return
            w = x2 - x1; set_sscale(-1)
            if (cache && cache.st_l == stafflines && staff_tb[st].staffscale == 1 && cache.st_w == (w | 0)) {
            ...
            if (!cache && w > get_lwidth() - 10 && staff_tb[st].staffscale == 1) {
                cache = { st_l: stafflines, st_w: w | 0 }
        ...
        function draw_bar(s, bot, h) {
            var i, s2, yb, w, bar_type = s.bar_type, st = s.st, p_staff = staff_tb[st], x = s.x
            ...
            if (s.bar_mrep) {
                set_sscale(st)
                if (s.bar_mrep == 1) { for (s2 = s.prev; s2.type != C.REST; s2 = s2.prev); xygl(s2.x, yb, "mrep") } else {
                    xygl(x, yb, "mrep2")
            ...
            for (i = bar_type.length; --i >= 0;) {
                switch (bar_type[i]) {
                    case "|": if (s.bar_dotted) { w = (5 * p_staff.staffscale).toFixed(1); out_XYAB('<path class="bW" stroke-dasharray="A,A" d="MX Yv-G"/>\n', x, bot, w, h) } else if (s.color) { out_XYAB('<path class="bW" d="MX Yv-F"/>\n', x, bot, h) } else {
                        sb += 'M' + sx(x).toFixed(1)
        ...
        function hl_rest(s) {
            var j, p_st = staff_tb[s.st], i = 5 - s.nflags, x = s.x, y = s.y
            if (i < 6)
                return
            ...
            j = y / 6
            switch (i) {
                default: switch (p_st.stafflines[j + 1]) {
                    case '|': case '[': break
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            switch (s.type) {
                case C.STAVES: sy = s.sy
                    for (st = 0; st <= nstaff; st++) {
                        ...
                        if (sy.st_print[st] && cur_sy.staves[st] && sy.staves[st].stafflines == cur_sy.staves[st].stafflines)
                            continue
                        if (s.ts_prev.bar_type) { x2 = s.ts_prev.x } else {
                ...
                case C.BAR: if (s.invis || !s.bar_type || !cur_sy.st_print[s.st])
                    ...
                    break
                case C.STBRK: if (cur_sy.voices[s.v] && cur_sy.voices[s.v].range == 0) {
                    if (s.xmx > 14 && s.next && s.next.type == C.CLEF) {
                        var nv = 0
    ...
    Abc.prototype.draw_symbols = function (p_voice) {
        var bm = {}, s, x, y, st; for (s = p_voice.sym; s; s = s.next) {
            if (s.invis) {
                switch (s.type) {
                    case C.CLEF: if (s.time >= staff_tb[s.st].clef.time)
    ...
    function draw_all_sym() {
        ...
        function draw_sl2() {
            ...
            while (1) {
                ...
                switch (sl[2]) {
                    case "//": case "\\\\": d = -(sl[1].prev.prev.y + staff_tb[sl[0].st].y
...
") format("truetype")'
    var font_scale_tb = { serif: 1, serifBold: 1, 'sans-serif': 1, 'sans-serifBold': 1, Palatino: 1.1, monospace: 1 }, txt_ff = "text,serif", fmt_lock = {}
    var cfmt = {
...
H "History: "', infospace: 0, keywarn: true, leftmargin: 1.4 * CM, lineskipfac: 1.1, linewarn: true, maxshrink: .65, maxstaffsep: 2000, maxsysstaffsep: 2000, measrepnb: 1, measurefont: { name: txt_ff, style: "italic", size: 10 }, measurenb: -1, musicfont: { name: "music", src: musicfont, size: 24 }, musicspace: 6, partsfont: { name: txt_ff, size: 15 }, parskipfac: .4, partsspace: 8, pagewidth: 21 * CM, "propagate-accidentals": "o", printmargin: 0, rightmargin: 1.4 * CM, rbmax: 4, rbmin: 2, repeatfont: { name: txt_ff, size: 9 }, scale: 1, slurheight: 1.0, spatab: new Float32Array([10.2, 13.3, 17.3, 22.48, 29.2, 38, 49.4, 64.2, 83.5, 108.5]), staffsep: 46, stemheight: 21, stretchlast: .25, stretchstaff: true, subtitlefont: { name: txt_ff, size: 16 }, subtitlespace: 3, sysstaffsep: 34, systnames: -1, tempofont: { name: txt_ff, weight: "bold", size: 12 }, textfont: { name: txt_ff, size: 16 }, textspace: 14, tieheight: 1.0, titlefont: { name: txt_ff, size: 20 }, titlespace: 6, titletrim: true, topspace: 22, tuplets: [0, 0, 0, 0], tupletfont: { name: txt_ff, style: "italic", size: 10 }, vocalfont: { name: txt_ff, weight: "bold", size: 13 }, vocalspace: 10, voicefont: { name: txt_ff, weight: "bold", size: 13 }, writefields: "CMOPQsTWw", wordsfont: { name: txt_ff, size: 16 }, wordsspace: 5, "writeout-accidentals": "n"
    ...
    function param_set_font(xxxfont, p) {
        ...
        if ((p[0] == 'u' && p.slice(0, 4) == "url(") || (p[0] == 'l' && p.slice(0, 6) == "local(")) {
            n = p.indexOf(')', 1)
            if (n < 0) {
        ...
        if (!font.name || !font.size) {
            ft2 = cfmt[xxxfont]
            for (k in ft2) {
                if (!ft2.hasOwnProperty(k) || font[k] != undefined)
                    continue
                switch (k) {
                    case "fid": case "used": case "src": break
    ...
    Abc.prototype.set_format = function (cmd, param) {
        ...
        switch (cmd) {
            case "aligncomposer": case "barsperstaff": case "infoline": case "measurenb": case "rbmax": case "rbmin": case "measrepnb": case "shiftunison": case "systnames": v = parseInt(param)
            ...
            case "bardef": v = param.split(/\s+/)
                if (v.length != 2) { syntax(1, errs.bad_val, "%%bardef") } else {
                    if (parse.ufmt)
            ...
            case "notespacingfactor": v = param.match(/([.\d]+)[,\s]*(\d+)?/)
                if (v) {
                    f = +v[1]
                    if (isNaN(f) || f < 1 || f > 2) { f = 0 } else if (v[2]) {
                        f2 = +v[2]
                        if (isNaN(f))
                            f = 0
                    } else { f2 = cfmt.spatab[5] }
                }
                if (!f) {
                    syntax(1, errs.bad_val, "%%" + cmd)
                    break
            ...
            case "pos": cmd = param.match(/(\w*)\s+(.*)/)
                ...
                if (cmd[1].slice(0, 3) == 'tup' && curvoice) {
                    ...
                    v = posval[cmd[2]]
                    switch (v) {
                        case C.SL_ABOVE: curvoice.tup[3] = 1
                            break
    ...
    function use_font(font) {
        if (!font.used) {
            font.used = true; if (font.fid == undefined) {
                font.fid = abc2svg.font_tb.length
                abc2svg.font_tb.push(font)
                if (!font.swfac)
                    set_font_fac(font)
 ...
 src:"+ font.src + "}")
        ...
        while (1) {
            ...
            switch (c) {
                case '0': case '2': if (src[i + 1] != '0')
                    ...
                    break
                case 'u': j = Number("0x" + src.slice(i + 1, i + 5)); if (isNaN(j) || j < 0x20) {
                    dst += src[++i] + "\u0306"
                    j = i + 1
                    continue
                }
    ...
    function tosvg(in_fname, file, bol, eof) {
        ...
        function do_voice(select, in_tune) {
            ...
            if (in_tune) {
                if (!parse.tune_v_opts)
                    parse.tune_v_opts = {}; opt = parse.tune_v_opts
            } else {
                if (!parse.voice_opts)
        ...
        function tune_filter() {
            ...
            for (i in parse.tune_opts) {
                ...
                for (j = 0; j < opts.t_opts.length; j++) {
                    pc = opts.t_opts[j]
                    switch (pc.match(/\S+/)[0]) {
                        case "score": case "staves": if (!parse.scores)
        ...
        for (; bol < eof; bol = parse.eol + 1) {
            ...
            eol++
            if (eol == bol) {
                if (parse.state == 1) { parse.istart = bol; syntax(1, "Empty line in tune header - ignored") } else if (parse.state >= 2) {
                    end_tune()
                    parse.state = 0
                    if (parse.select) {
                        eol = file.indexOf('\nX:', parse.eol)
            ...
            if (pscom) {
                ...
                if (!a || a[1][0] == '%')
                    continue
                switch (a[1]) {
                    case "abcm2ps": case "ss-pref": parse.prefix = a[2]
                        continue
                ...
                if (a[1].slice(0, 5) == 'begin') {
                    ...
                    if (i < 0) {
                        syntax(1, "No $1 after %%$2", end.slice(1), a[1]); parse.eol = eof
                        continue
                    }
                ...
                switch (a[1]) {
                    ...
                    case "tune": if (parse.state != 0) {
                        ...
                        if (select == "end")
                            continue
                        if (!parse.tune_opts)
                            parse.tune_opts = {}; parse.tune_opts[select] = opt = { t_opts: [] }; while (1) {
                                bol = eol
                                if (file[bol + 1] != '%')
                                    break
                                eol = file.indexOf('\n', eol + 1)
            ...
            if (line1 != ':' || !/[A-Za-z+]/.test(line0)) {
                last_info = undefined; if (parse.state < 2)
                    continue
            ...
            switch (line0) {
                ...
                case '|': if (parse.state < 2)
                    ...
                    parse_music_line()
                    continue
                default: if ("ABCDFGHNOSZ".indexOf(line0) >= 0) {
                    if (parse.state >= 2) {
                        syntax(1, errs.ignored, line0)
                        continue
                    }
    ...
    function identify_note(s, dur_o) {
        ...
        while (dur >> dots > 0)
            dots++
        flags -= dots
        if (flags >= 0) { head = C.FULL } else switch (flags) {
            default: error(1, s, "Note too long")
                flags = -4
    ...
    function set_head_shift(s) {
        ...
        for (i = i1; i != i2; i += dir) {
            ...
            if (d < 0)
                d = -d
            if (d > 3 || (d >= 2 && s.head != C.SQUARE)) { shift = false } else {
                shift = !shift
                if (shift) {
                    s.notes[i].shhd = dx
                    if (dx_max < dx)
    ...
    function lktsym(s, next) {
        ...
        if (next) {
            ...
            if (s.ts_prev)
                s.ts_prev.ts_next = s; next.ts_prev = s
        } else {
        ...
        if (next.seqst) {
            ...
            } else { next.shrink += next.wl - old_wl }
            next.space = 0
        }
    }
    function unlksym(s) {
        ...
        if (s.prev)
            ...
            s.p_v.sym = s.next
        if (s.ts_next) {
            if (s.seqst) { if (s.ts_next.seqst) { s.ts_next.shrink += s.shrink; s.ts_next.space += s.space } else { s.ts_next.seqst = true; s.ts_next.shrink = s.shrink; s.ts_next.space = s.space } } else {
    ...
    function insert_clef(s, clef_type, clef_line) {
        var p_voice = s.p_v, new_s, st = s.st
    ...
    function set_graceoffs(s) {
        var next, m, dx, x, gspleft = s.fmt.gracespace[0], gspinside = s.fmt.gracespace[1], gspright = s.fmt.gracespace[2], g = s.extra; if (s.prev && s.prev.type == C.BAR)
    ...
    function set_w_chs(s) {
        var i, ch, w0, s0, dw, x = 0, n = 0
        set_font("vocal")
        for (; s; s = s.ts_next) {
            if (s.shrink) { x += s.shrink; n++ }
            if (s.a_ly)
                ly_set(s)
            if (!s.a_gch)
                continue
    ...
    Abc.prototype.set_width = function (s) {
        ...
        switch (s.type) {
            case C.NOTE: case C.REST: s.wr = wlnote = s.invis ? 0 : hw_tb[s.head]
                if (s.xmx > 0)
                    s.wr += s.xmx + 4; for (s2 = s.prev; s2; s2 = s2.prev) {
                        if (w_tb[s2.type])
                            break
                    }
                if (s2) {
                    switch (s2.type) {
                        case C.BAR: case C.CLEF: case C.KEY: case C.METER: wlnote += 3
                            break
                ...
                if (s.dots) {
                    if (s.wl == undefined)
                        switch (s.head) {
                            case C.SQUARE: case C.OVALBARS: s.xmx += 3
                                break
                            case C.OVAL: s.xmx += 1
                                break
                        }
            ...
            case C.BAR: bar_type = s.bar_type
                ...
                if (s.text && s.text.length < 4 && s.next && s.next.a_gch) {
                    set_font("repeat"); s.wr += strwh(s.text)[0] + 2
                    if (cfmt.measurenb > 0 & s.bar_num && s.bar_num % cfmt.measurenb)
                        s.wr += 4
                }
                return
            case C.CLEF: if (s.invis) {
                s.wl = s.wr = 1
                return
            }
            ...
            case C.METER: s.x_meter = []
                ...
                for (i = 0; i < s.a_meter.length; i++) {
                    meter = s.a_meter[i]
                    switch (meter.top[0]) {
                        case 'C': case 'c': case 'o': s.x_meter[i] = wlw + 6; wlw += 12
                            break
                        ...
                        default: w = 0
                            if (!meter.bot || meter.top.length > meter.bot.length)
                                ...
                                meter = meter.bot; for (m = 0; m < meter.length; m++) {
                                    switch (meter[m]) {
                                        case '(': wlw += 4
                                        case ')': case '1': w += 4
                                            break
    ...
    function time2space(s, len) {
        ...
        if (smallest_duration >= C.BLEN / 2) {
            if (smallest_duration >= C.BLEN)
                ...
                len /= 2
        } else if (!s.next && len >= C.BLEN) { len /= 2 }
        if (len >= C.BLEN / 4) {
            if (len < C.BLEN / 2)
                i = 5
            else if (len < C.BLEN)
        ...
        space = cfmt.spatab[i]
        if (l) {
            if (l < 0) { space = cfmt.spatab[0] * len / (C.BLEN / 16 / 8) } else {
                if (i >= 9)
                    i = 8
    ...
    function set_space(s, ptime) {
        ...
        len = s.time - ptime
        if (!len) {
            switch (s.type) { case C.MREST: return s.wl }
            return 0
        }
    ...
    function set_sp_tup(s, s_et) {
        var tim = s.time, ttim = s_et.time - tim, sp = time2space(s, ttim), s2 = s, wsp = 0
        while (1) {
            s2 = s2.ts_next
            if (s2.seqst) {
                wsp += s2.space
    ...
    function set_allsymwidth(first) {
        var val, st, s_chs, stup, itup, s = tsfirst, s2 = s, xa = 0, xl = [], wr = [], maxx = xa, tim = s.time
        while (1) {
            itup = 0
            do {
                ...
                if (xl[st] == undefined)
                    xl[st] = 0
                if (wr[st] == undefined)
                    wr[st] = 0; if (s.prev && s.prev.st != st) {
                        xl[st] = xl[s.prev.st]
                        wr[st] = wr[s.prev.st]
                    }
                ...
                if (val > maxx)
                    maxx = val
                if (s.dur && s.dur != s.notes[0].dur && first)
                    itup = 1
                s = s.ts_next
            } while (s && !s.seqst); s2.shrink = maxx - xa
            ...
            if (!s2.shrink) { if (s2.type == C.CLEF && !s2.ts_prev.bar_type) { delete s2.seqst; s2.time = tim } else { s2.shrink = 10 } }
            tim = s2.time
            if (!s)
                break
            s = s2
    ...
    function set_repeat(s) {
        var s2, s3, i, j, dur, n = s.repeat_n, k = s.repeat_k, st = s.st, v = s.v
        s.repeat_n = 0
        if (n < 0) {
            n = -n; i = n
            ...
            for (j = k; --j >= 0;) {
                ...
                s.head = C.SQUARE; for (s = s2; s; s = s.ts_next) {
        ...
        if (n == 2) {
            ...
            s2.bar_mrep = 2
            s3 = s2.next; for (s2 = s3.ts_next; ; s2 = s2.ts_next) {
                if (s2.st != st)
                    continue
                if (s2.type == C.BAR) {
                    if (s2.v == v)
    ...
    function set_nl(s) {
        ...
        function bardiv(so) {
            ...
            function eol_bar(s, so, sst) {
                ...
                if (!s2) {
                    ...
                    if (s == sst)
                        s2.seqst = 1
                    if (s2.seqst) {
                        for (s = s2.ts_next; !s.seqst; s = s.ts_next); s2.shrink = s.shrink
                        s.shrink = s2.wr + s.wl
                        s2.space = s.space
                        s.space = 0
                    }
            ...
            for (so = s1; so.time == s1.time; so = so.ts_prev) {
                switch (so.ts_prev.type) { case C.KEY: case C.METER: case C.TEMPO: case C.STAVES: case C.STBRK: continue }
                break
            }
            s = s1
            while (1) {
        ...
        function do_warn(s) {
            ...
            for (s2 = s; s2 && s2.time == s.time; s2 = s2.ts_next) {
                switch (s2.type) {
                    case C.KEY: if (!s.fmt.keywarn || s2.invis)
                        continue
                        for (s1 = s.ts_prev; s1; s1 = s1.ts_prev) {
                            if (s1.type != C.METER)
                                break
                        }
                    case C.METER: if (s2.type == C.METER) {
                        if (!s.fmt.timewarn)
                            continue
                        s1 = s.ts_prev
                    }
    ...
    function get_width(s, next) {
        var shrink, space, w = 0, wmx = 0, sp_fac = (1 - s.fmt.maxshrink)
        while (s != next) {
            if (s.seqst) {
                shrink = s.shrink
                wmx += shrink
    ...
    function set_lines(s, next, lwidth, indent) {
        var first, s2, s3, s4, s5, x, xmin, xmid, xmax, wwidth, shrink, space, nlines, last = next ? next.ts_prev : null, ws = get_width(s, next)
        ...
        while (1) {
            ...
            s2 = first = s; xmin = s.x - s.shrink - indent; xmax = xmin + lwidth; xmid = xmin + wwidth / nlines; xmin += wwidth / nlines * s.fmt.breaklimit; for (s = s.ts_next; s != next; s = s.ts_next) {
                if (!s.x)
                    continue
                if (s.type == C.BAR)
                    s2 = s
            ...
            if (s.nl) {
                error(0, s, "Line split problem - adjust maxshrink and/or breaklimit"); nlines = 2
    ...
    function cut_tune(lwidth, lsh) {
        var s2, i, mc, pg_sav = { leftmargin: cfmt.leftmargin, rightmargin: cfmt.rightmargin, pagewidth: cfmt.pagewidth, scale: cfmt.scale }, indent = lsh[0] - lsh[1], ckw = get_ck_width(), s = tsfirst
        lwidth -= lsh[1]
        ...
        for (; s; s = s.ts_next) {
            if (s.type == C.BLOCK) {
                switch (s.subtype) {
                    case "leftmargin": case "rightmargin": case "pagescale": case "pagewidth": case "scale": case "staffwidth": if (!s.soln)
            ...
            if (!s.ts_next) { s = null } else if (!s.soln) { continue } else {
                s.soln = false
                if (s.time == s2.time)
                    continue
                while (!s.seqst)
    ...
    function set_yval(s) {
        switch (s.type) {
            case C.CLEF: if (s.second || s.invis) {
                s.ymx = s.ymn = 12
                break
            }
                s.y = (s.clef_line - 1) * 6
                switch (s.clef_type) {
                    default: s.ymx = s.y + 28
                        s.ymn = s.y - 14
                        break
                    case "c": s.ymx = s.y + 13
                        s.ymn = s.y - 11
    ...
    function set_ottava() {
        var s, s1, st, o, d, m = nstaff + 1, staff_d = new Int8Array(m)
        function sym_ott(s, d) {
            var g, m, note
            switch (s.type) {
                case C.REST: if (voice_tb.length == 1)
                    break
                case C.NOTE: if (!s.p_v.ckey.k_drum) {
                    for (m = s.nhd; m >= 0; m--) {
                        note = s.notes[m]; if (!note.opit)
    ...
    function mrest_expand() {
        var s, s2
        function mexp(s) {
            var bar, s3, s4, tim, nbar, nb = s.nmes, dur = s.dur / nb, s2 = s.next
            while (!s2.bar_type)
                s2 = s2.next
            bar = s2
            while (!s2.bar_num)
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.type != C.MREST)
                continue
            if (!s.seqst && w_tb[s.ts_prev.type]) { s2 = s } else {
                s2 = s.ts_next
                while (!s2.seqst) {
    ...
    function set_auto_clef(st, s_start, clef_type_start) {
        var s, time, s2, s3, max = 12, min = 20
        ...
        for (s = s_start; s != s_last; s = s.ts_next) {
            ...
            if (!s_last_chg) {
                clef_type = clef_type_start = clef_type == 't' ? 'b' : 't'; s_last_chg = s
                continue
            }
            s3 = s
    ...
    function set_clefs() {
        var s, s2, st, v, p_voice, g, new_type, new_line, p_staff, pit, staff_clef = new Array(nstaff + 1), sy = cur_sy, mid = []
    ...
    Abc.prototype.set_pitch = function (last_s) {
        var s, s2, g, st, delta, pitch, note, dur = C.BLEN, m = nstaff + 1, staff_delta = new Int16Array(m * 2), sy = cur_sy
        for (st = 0; st <= nstaff; st++) {
            s = staff_tb[st].clef; staff_delta[st] = delta_tb[s.clef_type] + s.clef_line * 2
            if (s.clefpit)
        ...
        for (s = tsfirst; s != last_s; s = s.ts_next) {
            st = s.st
            switch (s.type) {
                case C.CLEF: staff_delta[st] = delta_tb[s.clef_type] +
                    s.clef_line * 2
                    if (s.clefpit)
                        staff_delta[st] += s.clefpit
                    if (cfmt.sound) {
                ...
                case C.GRACE: for (g = s.extra; g; g = g.next) {
                    ...
                    if (delta && !s.p_v.ckey.k_drum) {
                        for (m = 0; m <= g.nhd; m++) {
                            note = g.notes[m]; note.opit = note.pit
                            note.pit += delta
                        }
                    }
    ...
    Abc.prototype.set_stem_dir = function () {
        var t, u, i, st, rvoice, v, v_st, st_v, vobj, v_st_tb, st_v_tb = [], s = tsfirst, sy = cur_sy, nst = sy.nstaff
        while (s) {
            for (st = 0; st <= nst; st++)
                st_v_tb[st] = []
            v_st_tb = []
            for (u = s; u; u = u.ts_next) {
                ...
                if (v_st.st1 < 0) { v_st.st1 = st } else if (v_st.st1 != st) {
                    if (st > v_st.st1) {
                        if (st > v_st.st2)
                            v_st.st2 = st
                    } else {
            ...
            for (; s != u; s = s.ts_next) {
                ...
                if (i < 0)
                    continue
                if (i == st_v.length - 1) { s.multi = -1 } else {
                    s.multi = 1
    ...
    function set_rest_offset() {
        var s, s2, v, v_s, ymax, ymin, d, v_s_tb = [], sy = cur_sy
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            switch (s.type) {
                ...
                case C.REST: if (s.invis || !s.multi)
                    continue
                    v_s = v_s_tb[s.v]
                    if (!v_s) { v_s_tb[s.v] = v_s = { d: 0 } } else if (v_s.d) {
                        s2 = v_s.s
                        d = loffs(v_s.d)
                        s2.y += d
                        s2.ymx += d
            ...
            for (v = 0; v < v_s_tb.length; v++) {
                ...
                if (sy.voices[v].range > sy.voices[s.v].range) {
                    ...
                    if (s2.ymn + v_s.d >= s.ymx)
                        continue
                    d = s.ymx - s2.ymn
                    if (s.type == C.REST && s2 == s.ts_prev && s.y == s2.y) {
                        if (!v_s_tb[s.v])
                            v_s_tb[s.v] = { d: 0 }
                        if (v_s_tb[s.v].d > -6)
                            v_s_tb[s.v].d = -6
                        d = 6
                    } else if (s2.time < s.time) { d = s.ymx - s2.y }
                    if (v_s.d) {
                        if (v_s.d < 0) {
                            rshift()
                            continue
    ...
    function init_music_line() {
        var p_voice, s, s1, s2, s3, last_s, v, st, shr, shrmx, shl, shlp, p_st, top, nv = voice_tb.length, fmt = tsfirst.fmt
        for (v = 0; v < nv; v++) {
            if (!cur_sy.voices[v])
                continue
            p_voice = voice_tb[v]; p_voice.st = cur_sy.voices[v].st
            p_voice.second = cur_sy.voices[v].second; p_voice.last_sym = p_voice.sym; for (s = p_voice.sym; s && s.time == tsfirst.time; s = s.next) {
                switch (s.type) {
                    case C.CLEF: case C.KEY: case C.METER: switch (s.type) {
                        case C.CLEF: staff_tb[s.st].clef = s
                            break
        ...
        if (insert_meter) {
            for (v = 0; v < nv; v++) {
                p_voice = voice_tb[v]; s2 = p_voice.meter
                if (!cur_sy.voices[v] || cur_sy.voices[v].second || !cur_sy.st_print[cur_sy.voices[v].st])
                    continue
                s = clone(s2)
                new_sym(s, p_voice, last_s)
        ...
        for (v = 0; v < nv; v++) {
            ...
            if (p_voice.sls.length) {
                s = { type: C.BAR, fname: last_s.fname, bar_type: "|", dur: 0, multi: 0, invis: true, sls: p_voice.sls, fmt: fmt }
                new_sym(s, p_voice, last_s)
                p_voice.sls = []
            }
        }
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v]; s2 = p_voice.bar_start; p_voice.bar_start = null
    ...
    function set_words(p_voice) {
        var s, s2, nflags, lastnote, res, start_flag = true, pitch = 127
        function trem_adj(s) {
            s.prev.trem2 = true
            s.prev.head = ++s.head
            if (--s.nflags > 0) { s.nflags += s.ntrem } else {
                if (s.nflags <= -2) {
                    s.stemless = true
            ...
            switch (s.type) {
                ...
                case C.NOTE: case C.REST: res = identify_note(s, s.dur_orig); s.head = res[0]; s.dots = res[1]; s.nflags = res[2]
                    ...
                    if (start_flag || nflags <= 0) {
                        if (lastnote) { lastnote.beam_end = true; lastnote = null }
    ...
    var delpit = [0, -7, -14, 0]
    function set_global() {
        var p_voice, v, nv = voice_tb.length, sy = cur_sy, st = sy.nstaff
        ...
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v]; set_words(p_voice)
    ...
    function get_lshift() {
        var st, v, p_v, p1, po, fnt, w, sy = cur_sy, lsh1 = 0, lsho = 0, nv = voice_tb.length
    ...
    function set_indent(lsh) {
        var st, v, w, p_voice, p, i, j, font, vnt = 0, fmt = tsnext ? tsnext.fmt : cfmt
        if (fmt.systnames) {
            for (v = voice_tb.length; --v >= 0;) {
                p_voice = voice_tb[v]
                if (!cur_sy.voices[v] || !gene.st_print[p_voice.st])
                    continue
                if (p_voice.nm && (p_voice.new_name || fmt.systnames == 2)) {
                    vnt = 2
                    break
                }
                if (p_voice.snm)
    ...
    function set_beams(sym) {
        ...
        for (s = sym; s; s = s.next) {
            ...
            if (!s.stem) {
                mid_p = s.mid / 3 + 18
                if (beam) { s.stem = laststem } else if (s.beam_st && !s.beam_end) {
                    beam = true; pu = s.notes[s.nhd].pit; pd = s.notes[0].pit
                    ...
                    if (!s.stem && g.beam_end) {
                        if (pu + pd < mid_p * 2) { s.stem = 1 } else if (pu + pd > mid_p * 2) { s.stem = -1 } else {
                            if (s.fmt.bstemdown)
                                s.stem = -1
                        }
                    }
            ...
            if (s.beam_end)
                beam = false; laststem = s.stem; if (s_opp) {
    ...
    var MAXPIT = 48 * 2
    function set_left(s) {
        var m, i, j, shift, w_base = w_note[s.head], w = w_base, left = []
        for (i = 0; i < MAXPIT; i++)
            left.push(-100)
        if (s.nflags > -2) {
            if (s.stem > 0) { w = -w; i = s.notes[0].pit * 2; j = (Math.ceil((s.ymx - 2) / 3) + 18) * 2 } else { i = (Math.ceil((s.ymn + 2) / 3) + 18) * 2; j = s.notes[s.nhd].pit * 2 }
            if (i < 0)
                i = 0
            if (j >= MAXPIT)
                j = MAXPIT - 1
        ...
        shift = s.notes[s.stem > 0 ? 0 : s.nhd].shhd; for (m = 0; m <= s.nhd; m++) {
    ...
    function set_right(s) {
        var m, i, j, k, shift, w_base = w_note[s.head], w = w_base, flags = s.nflags > 0 && s.beam_st && s.beam_end, right = []
        for (i = 0; i < MAXPIT; i++)
            right.push(-100)
        if (s.nflags > -2) {
            if (s.stem < 0) { w = -w; i = (Math.ceil((s.ymn + 2) / 3) + 18) * 2; j = s.notes[s.nhd].pit * 2; k = i + 4 } else { i = s.notes[0].pit * 2; j = (Math.ceil((s.ymx - 2) / 3) + 18) * 2 }
            if (i < 0)
                i = 0
            if (j > MAXPIT)
                j = MAXPIT
            ...
            while (1) {
                ...
                switch (dp) {
                    ...
                    case 1: if (s1.notes[i1].pit & 1)
                        ...
                        s1.dot_low = false
                        break
                    case 0: if (s1.notes[i1].acc != s2.notes[i2].acc) {
                        t = -1
                        break
                    }
                        if (s2.notes[i2].acc) {
    ...
    Abc.prototype.set_stems = function () {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if (s.ntrem)
                slen += 2 * s.ntrem
            if (s.stemless) {
                if (s.stem >= 0) { s.y = ymn; s.ys = ymx } else { s.ys = ymn; s.y = ymx }
                s.ymx = ymx + 4; s.ymn = ymn - 4
            } else if (s.stem >= 0) {
    ...
    Abc.prototype.block_gen = function (s) {
        switch (s.subtype) {
            case "leftmargin": case "rightmargin": case "pagescale": case "pagewidth": case "scale": case "staffwidth": self.set_format(s.subtype, s.param)
                break
            case "mc_start": if (multicol) {
                ...
                break
            case "mc_new": if (!multicol || multicol.state != parse.state) {
                ...
                img.chg = 1
                break
            case "mc_end": if (!multicol || multicol.state != parse.state) {
    ...
    function set_piece() {
        var s, last, p_voice, st, v, nv, tmp, non_empty, non_empty_gl = [], sy = cur_sy
        ...
        function set_top_bot() {
            ...
            for (st = 0; st <= nstaff; st++) {
                ...
                for (i = 0; i < l - 1; i++) {
                    switch (p_staff.stafflines[i]) { case '.': case '-': continue }
                    break
                }
                p_staff.botline = p_staff.botbar = i * 6
                if (i >= l - 2) {
                    if (p_staff.stafflines[i] != '.') { p_staff.botbar -= 6; p_staff.topbar += 6 } else {
                        p_staff.botbar -= 12; p_staff.topbar += 12
                        continue
                    }
                }
                if (!non_empty_gl[st])
                ...
                for (j = 1; i < l; i++, j += 2) {
                    switch (p_staff.stafflines[i]) {
                        case '|': case '[': p_staff.hlmap[j - 1] = 1; p_staff.hlmap[j] = 1; p_staff.hlmap[j + 1] = 1
                            break
                    }
                }
            }
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.nl)
                break
            switch (s.type) {
                case C.STAVES: set_brace(); sy.st_print = non_empty
            ...
            if (st > nstaff) {
                switch (s.type) {
                    case C.CLEF: staff_tb[st].clef = s
                        break
                    case C.KEY: s.p_v.ckey = s
                        break
                    case C.METER: s.p_v.meter = s
        ...
        set_top_bot()
        if (tsnext) {
            s = tsnext; delete s.nl; last = s.ts_prev; last.ts_next = null; nv = voice_tb.length
            for (v = 0; v < nv; v++) {
                p_voice = voice_tb[v]
                if (p_voice.sym && p_voice.sym.time <= tsnext.time) {
                    for (s = last; s; s = s.ts_prev) { if (s.v == v) { p_voice.s_next = s.next; s.next = null; break } }
                    if (s)
                        continue
                }
    ...
    Abc.prototype.set_sym_glue = function (width) {
        var g, x, some_grace, stretch, cnt = 4, xmin = 0, xx = 0, xs = 0, xse = 0, ll = !tsnext || (tsnext.type == C.BLOCK && !tsnext.play) || blocks.length, s = tsfirst, spf = 1, xx0 = 0
        for (; s; s = s.ts_next) {
            ...
            if (s.seqst) {
                ...
                if (s.space) {
                    if (s.space < s.shrink) { xse += s.shrink; xx += s.shrink } else {
                        xx += s.space
                        xx0 += s.shrink
                    }
                } else { xs += s.shrink }
        ...
        if (ll) {
            if ((xx - xx0 + xs) / width > (1 - s.fmt.stretchlast))
                stretch = 1
        } else if (s.fmt.stretchstaff) { stretch = 1 }
        if (xmin >= width) {
            x = 0
            for (; s; s = s.ts_next) {
                if (s.seqst)
            ...
            while (--cnt >= 0) {
                xx = 0; xse = 0; x = 0
                for (s = tsfirst; s; s = s.ts_next) {
                    if (s.seqst) { if (s.space) { if (s.space * spf <= s.shrink) { xse += s.shrink; xx += s.shrink; x += s.shrink } else { xx += s.space; x += s.space * spf } } else { x += s.shrink } }
                    s.x = x
                }
                if (!stretch && x < width)
                    break
    ...
    function set_sym_line() {
        ...
        while (--v >= 0) {
            ...
            if (s) {
                ...
                p_v.s_prev = s.prev
                s.prev = null
            } else { p_v.s_prev = null }
        }
    }
    function set_posx() { posx = img.lm / cfmt.scale }
    function gen_init() {
        var s = tsfirst, tim = s.time
        for (; s; s = s.ts_next) {
            if (s.time != tim) {
                set_page()
                return
            ...
            switch (s.type) {
                case C.NOTE: case C.REST: case C.MREST: case C.SPACE: set_page()
                    return
                default: continue
    ...
    Abc.prototype.output_music = function () {
        ...
        lsh = get_lshift()
        if (cfmt.singleline) {
            v = get_ck_width(); lwidth = lsh[0] + v[0] + v[1] + get_width(tsfirst, null)[0]
            v = cfmt.singleline == 2 ? get_lwidth() : lwidth
            if (v > lwidth)
                lwidth = v
            else
                img.width = lwidth * cfmt.scale + img.lm + img.rm + 2
        } else { lwidth = get_lwidth(); cut_tune(lwidth, lsh) }
        ts1st = tsfirst
        v = nv
        while (--v >= 0)
        ...
        spf_last = .65
        while (1) {
            set_piece(); indent = set_indent(lsh)
    ...
    function new_clef(clef_def) {
        var s = { type: C.CLEF, clef_line: 2, clef_type: "t", v: curvoice.v, p_v: curvoice, time: curvoice.time, dur: 0 }, i = 1
        set_ref(s)
        switch (clef_def[0]) {
            case '"': i = clef_def.indexOf('"', 1); s.clef_name = clef_def.slice(1, i); i++
                break
            case 'a': if (clef_def[1] == 'u') {
    ...
    function set_user(parm) {
        ...
        switch (char_tb[k][0]) {
            case '0': case 'd': case 'i': case ' ': break
    ...
    function new_block(subtype) {
        var s = { type: C.BLOCK, subtype: subtype, dur: 0 }
        sym_link(s)
        return s
    }
    Abc.prototype.set_vp = function (a) {
        var s, item, pos, val, clefpit, tr_p = 0
        while (1) {
            item = a.shift()
            if (!item)
                break
            ...
            switch (item) {
                ...
                case "tacet=": val = a.shift()
                    ...
                    break
                case "transpose=": if (cfmt.nedo) {
                    syntax(1, errs.notransp)
                    break
                }
                    val = get_transp(a.shift())
                    if (val == undefined) { syntax(1, errs.bad_transp) } else {
                        curvoice.sound = val
                        if (cfmt.sound)
                            curvoice.score = val
                        tr_p = 2
                    ...
                    break
                default: switch (item.slice(0, 4)) {
                    case "treb": case "bass": case "alto": case "teno": case "perc": s = item
                        break
    ...
    function new_key(param) {
        var i, key_end, c, tmp, note, sf = "FCGDAEB".indexOf(param[0]) - 1, mode = 0, s = { type: C.KEY, dur: 0 }
        set_ref(s); i = 1
        if (sf < -1) {
            switch (param[0]) {
                case 'H': key_end = true
                ...
                case 'P': syntax(1, "K:P is deprecated"); sf = 0; s.k_drum = true; key_end = true
                    break
                case 'n': if (param.indexOf("none") == 0) {
                    sf = 0; s.k_none = true; i = 4
                    break
                }
                default: s.k_map = []
                    s.k_mode = 0
        ...
        if (!key_end) {
            ...
            c = param[0]
            if (c == '^' || c == '_' || c == '=') {
                s.k_a_acc = []; tmp = new scanBuf; tmp.buffer = param
                do {
                    ...
                    while (c == ' ')
                        c = param[++tmp.index]
                } while (c == '^' || c == '_' || c == '='); param = param.slice(tmp.index)
    ...
    function new_meter(p) {
        var p_v, s = { type: C.METER, dur: 0, a_meter: [] }, meter = {}, val, v, m1 = 0, m2, i = 0, j, wmeasure, in_parenth; set_ref(s)
        if (p.indexOf("none") == 0) { i = 4; wmeasure = 1 } else {
            wmeasure = 0
            while (i < p.length) {
                if (p[i] == '=')
                    break
                switch (p[i]) {
                    case 'C': meter.top = p[i++]
                        if (!m1) { m1 = 4; m2 = 4 }
                        break
                    case 'c': case 'o': meter.top = p[i++]
                        if (!m1) {
                            if (p[i - 1] == 'c') { m1 = 2; m2 = 4 } else { m1 = 3; m2 = 4 }
                            switch (p[i]) {
                                case '|': m2 /= 2
                                    break
                                case '.': m1 *= 3; m2 *= 2
        ...
        if (parse.state != 3) {
            ...
            if (parse.state) {
                ...
                for (v = 0; v < voice_tb.length; v++) { voice_tb[v].meter = s; voice_tb[v].wmeasure = wmeasure }
            }
        } else {
    ...
    function link_pq(s, text) {
        var p_v, s2
        if (curvoice.v == par_sy.top_voice) { sym_link(s) } else if (voice_tb[par_sy.top_voice].time == s.time) {
            p_v = curvoice
    ...
    function new_tempo(text) {
        var i, c, d, nd, txt = text, s = { type: C.TEMPO, dur: 0 }
        function get_nd(p) {
    ...
    function adjust_dur(s) {
        var s2, time, auto_time, i, fac; s2 = curvoice.last_sym
        if (!s2)
    ...
    Abc.prototype.new_note = function (grace, sls) {
        var note, s, in_chord, c, dcn, type, tie_s, acc_tie, i, n, s2, nd, res, num, dur, apit, div, ty, dpit = 0, sl1 = [], line = parse.line, a_dcn_sav = a_dcn
        a_dcn = []
        ...
        switch (c) {
            ...
            default: if (curvoice.acc_tie) {
                ...
                while (1) {
                    ...
                    if (c == ']') {
                        line.index++; nd = parse_dur(line); s.nhd = s.notes.length - 1
        ...
        if (typeof document != "undefined") {
            ...
            strwh = function (str) {
                ...
                while (1) {
                    ...
                    if (i < 0)
                        break
                    el.style.font = style_font(font).slice(5); i += 2; i0 = i
                }
                return [w, h]
            }
        } else {
    ...
    function write_headform(lwidth) {
        var c, font, font_name, align, x, y, sz, w, yd, info_val = {}, info_font = Object.create(info_font_init), info_sz = { A: cfmt.infospace, C: cfmt.composerspace, O: cfmt.composerspace, R: cfmt.infospace }, info_nb = {}
...
\n.box{outline:1px solid black;outline-offset:1px}', font_style = '', posx = cfmt.leftmargin / cfmt.scale, posy = 0, img = { width: cfmt.pagewidth, lm: cfmt.leftmargin, rm: cfmt.rightmargin, wx: 0, chg: 1 }, defined_glyph = {}, defs = '', fulldefs = '', stv_g = { scale: 1, dy: 0, st: -1, v: -1, g: 0 }, blkdiv = 0
...
<tspan dy=".5" style="font-size:10px">mb</tspan></text>\n', x - 10, y); x += 18
 ...
 fill="currentColor" stroke-width=".7"', g = ''
        ...
        if (psvg)
            psvg.ps_flush(true); if (blkdiv > 0) {
    ...
    function sort_all() {
        ...
        function b_chk() {
            ...
            if (!fl) {
                while (prev.type == C.GRACE && vtb[prev.v] && !vtb[prev.v].bar_type) {
                    delete prev.seqst
                    vtb[prev.v] = prev
                    prev = prev.ts_prev
                    fl = 1
    ...
    function get_map(text) {
        ...
        ns = a[1]
        if (ns[0] == '*' || ns.indexOf("all") == 0) { ns = 'all' } else {
    ...
    function get_staves(cmd, parm) {
        var s, p_voice, p_voice2, i, flags, v, vid, a_vf, st, range, nv = voice_tb.length, maxtime = 0
        if (curvoice && curvoice.clone) {
            i = parse.eol
            parse.eol = parse.bol
            do_cloning()
        ...
        for (i = 0; i < a_vf.length; i++) {
            vid = a_vf[i][0]; p_voice = new_voice(vid); p_voice.time = maxtime; v = p_voice.v
        ...
        for (i = 0; i < a_vf.length; i++) {
            ...
            if (flags & FL_VOICE) { p_voice.floating = true; p_voice.second = true } else {
                st++; if (!par_sy.staves[st]) { par_sy.staves[st] = { stafflines: p_voice.stafflines || "|||||", staffscale: 1 } }
    ...
    function do_cloning() {
        var i, clone = curvoice.clone, vs = clone.vs, a = clone.a, bol = clone.bol, eol = parse.eol, parse_sav = parse, file = parse.file
    ...
    function ly_set(s) {
        var i, j, ly, d, s1, s2, p, w, spw, xx, sz, shift, dw, s3 = s, wx = 0, wl = 0, n = 0, dx = 0, a_ly = s.a_ly, align = 0
        for (s2 = s.ts_next; s2; s2 = s2.ts_next) {
            if (s2.shrink) {
                dx += s2.shrink
                n++
        ...
        for (i = 0; i < a_ly.length; i++) {
            ...
            w = ly.t.wh[0]
            if (s.type == C.GRACE) { shift = s.wl } else if ((p[0] >= '0' && p[0] <= '9' && p.length > 2) || p[1] == ':' || p[0] == '(' || p[0] == ')') {
                if (p[0] == '(') { sz = spw } else {
                    j = p.indexOf(' ')
                    set_font(ly.font)
                    if (j > 0)
    ...
    Abc.prototype.cfmt = function () { return cfmt }; Abc.prototype.clone = clone; Abc.prototype.deco_put = function (nm, s) {
        a_dcn.push(nm)
        deco_cnv(s)
    }
...
abc2svg.modules = {
    ambitus: {}, begingrid: { fn: 'grid3' }, beginps: { fn: 'psvg' }, break: {}, capo: {}, chordnames: {}, clip: {}, clairnote: { fn: 'clair' }, voicecombine: { fn: 'combine' }, diagram: { fn: 'diag' }, equalbars: {}, gamelan: {}, grid: {}, grid2: {}, jazzchord: {}, jianpu: {}, mdnn: {}, MIDI: {}, nns: {}, pageheight: { fn: 'page' }, pedline: {}, percmap: { fn: 'perc' }, roman: {}, soloffs: {}, sth: {}, strtab: {}, temperament: { fn: 'temper' }, tropt: {}, nreq: 0, load: function (file, relay, errmsg) {
        function get_errmsg() {
...
abc2svg.version = "v1.22.14-22-0cb477e642"; abc2svg.vdate = "2024-04-07"
export default Abc